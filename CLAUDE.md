...
19. (2026-09-01) Pillo (pillo-app.framer.website) analyzed for animation/effects inspiration, user scoped the build via AskUserQuestion: (a) custom cursor — sitewide, added in `liquid.css`/`liquid.js` (gold dot + lagging ring, scales+brightens over `a,button,[data-tilt],[data-magnetic],input,select,textarea,.faq-q,.sim-frame,.flow-tab,.sim-dot,.sim-edge,.theme-btn,.burger`; gated behind `hover:hover and pointer:fine` + `!reduce`, `<html class="has-custom-cursor">` toggles `cursor:none`; ring lag uses a plain CSS `transition:left/top .16s linear` on direct style assignment, deliberately **not** a `requestAnimationFrame` lerp loop — rAF is throttled/paused on backgrounded tabs and produced a visibly stuck cursor during testing); (b) scroll-pinned product demo — `/work/` only, on the 3 real `.sim` case studies (hr-payroll, retail-ops, ride-hailing): a new ScrollTrigger block (`pin: [.case-visual], pinSpacing:false, scrub:0.6`) pins the phone in place while its case-study article scrolls past, `onUpdate` maps scroll progress across the pin range to `sim.__go(i)` (the existing tap/dot/swipe handlers are unchanged and still work between scroll ticks) — desktop-only (`innerWidth>980`) and skipped under `prefers-reduced-motion`. `.sim{max-width:min(280px,29vh)}` — the vh term is a deliberate fix, not decorative: at fixed 280px the pinned phone overflowed the bottom of shorter viewports (own testing caught 3/3 cases overflowing by up to 77px at a 720px-tall viewport) since GSAP's `pin` freezes the element at whatever vertical offset it naturally had in the document flow rather than centering it in the viewport; shrinking against `vh` only kicks in on short viewports and leaves the demo at full 280px on normal ones. (c) Investigated but **not** ported: Pillo's tight letter-spacing and frosted-glass secondary buttons were already matched or exceeded by the existing `.display/.h1/.h2` (`-.04em`/`-.035em`/`-.032em`, tighter than Pillo's `-1px`≈`-.02em`) and `.btn-glass` (`backdrop-filter:blur(14px)` + translucent `var(--glass2)`, already implemented) — verified via computed-style extraction before touching anything, so no redundant edit was made. Pillo's light→dark section mood-shift was judged not portable (SCS dark theme is already near-black by default; there's no "light hero" to shift away from) and was intentionally not forced in.
   **Tooling note for future sessions**: this browser-automation pane is unreliable for screenshotting scroll-position-dependent state — `window.scrollTo()` (vs real wheel-scroll) desyncs GSAP ScrollTrigger, and the pane reports `document.hidden:true` / `innerWidth:0` / paused `requestAnimationFrame` whenever backgrounded, which also broke verification of the Pillo reference site itself. The reliable fallback used throughout: verify via `getComputedStyle`/`getBoundingClientRect`/GSAP's own `ScrollTrigger.getAll()[i].progress` instead of trusting screenshots for anything scroll-linked.
